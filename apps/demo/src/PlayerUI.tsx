...
export function PlayerUI({ phase, onButtonClick, videoRef }: PlayerUIProps) {
  const btnLabel = match(phase)
    .with("init", () => "Play")
    .with("playing", () => "Pause")
    .with("paused", () => "Resume")
    .exhaustive();
      ...
      <Box sx={{ m: "20px" }}>
        <Box sx={{ display: "flex", gap: 2, flexDirection: { xs: "column", lg: "row" } }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Button variant="contained" color="info" fullWidth onClick={onButtonClick}>
              {btnLabel}
            </Button>
